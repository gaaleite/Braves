...
def carregar_dados():
    try:
        # Link público direto da aba correta formatado para CSV do Google Sheets
        url_csv = "https://google.com"
        
        # Carrega tratando as linhas e sem forçar header nulo se a planilha já tiver cabeçalho
        ...
        # Mapeamento exato baseado na estrutura oficial da planilha
        if qtd_colunas >= 1:
        ...
        # Filtra apenas linhas onde o ID do jogo é numérico válido
        ...
        # Remove as colunas temporárias de texto raw
...
if not df_jogos.empty:
    ...
    if not df_filtrado.empty:
        ...
        # Conclusão do fechamento dos eixos pendente no script original
        fig.update_xaxes(title_text="Temporada / Partida", tickangle=0)
        fig.update_yaxes(showticklabels=False, showgrid=False)
        
        st.plotly_chart(fig, use_container_width=True)
        
        # Exibe a tabela formatada abaixo do gráfico para conferência rápida
        st.write("### 📋 Tabela de Dados Filtrada")
        st.dataframe(df_filtrado[["ID_JOGO", "DATA", "ANO", "TORNEIO", "CATEGORIA", "CIDADE", "VD", "PP", "PC", "ADVERSARIO"]], use_container_width=True)
    else:
