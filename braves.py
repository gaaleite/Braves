...
def carregar_dados_limpos(url):
    try:
        # Lê o CSV ignorando linhas com erro de parsing de forma segura
        ...
        if not df.empty:
            ...
            # MAPEAMENTO EXATO BASEADO NAS POSIÇÕES QUE VOCÊ PASSOU
            ...
            # Limpeza preventiva contra os códigos estranhos e linhas vazias
            if "PP_INTERNA" in df.columns and "PC_INTERNA" in df.columns:
                df["PP_INTERNA"] = pd.to_numeric(df["PP_INTERNA"], errors='coerce')
                df["PC_INTERNA"] = pd.to_numeric(df["PC_INTERNA"], errors='coerce')
                df = df.dropna(subset=["PP_INTERNA", "PC_INTERNA"])
...
# Carrega os dados tratados e protegidos contra falhas
df_todos_jogos = carregar_dados_limpos(URL_NORMAL)

# Nomes padronizados das variáveis internas para consistência do filtro
    ...
    with tabs[i]:
        ...
        if nome_da_aba == "ALL GAMES":
            if df_todos_jogos.empty:
                st.warning("Nenhum dado encontrado ou a planilha está inacessível.")
            else:
                st.write("### 🔍 Filtros de Pesquisa")
                st.caption("Deixe os campos em branco para ver todos os dados. Digite para filtrar o gráfico em tempo real.")
                ...
                if not df_filtrado.empty:
                    ...
                    # Constrói o identificador de cada ponto no gráfico de forma amigável
                    jogo_str = df_filtrado[col_jogo].astype(str).str.strip()
                    time_str = df_filtrado[col_time].astype(str).str.strip()
                    adv_str = df_filtrado[col_adv].astype(str).str.strip()
                    
                    df_filtrado["Partida"] = "J" + jogo_str + " - " + time_str + " vs " + adv_str
                    
                    # Filtro final rígido contra qualquer resquício de código do Google Sheets
                    df_filtrado = df_filtrado[~df_filtrado["Partida"].str.contains(r"\{|function|var", case=False, na=True)]
                    
                    if not df_filtrado.empty:
                        # GERAÇÃO DO GRÁFICO DE LINHAS INTERATIVO
                        fig = px.line(
                            df_filtrado,
                            x="Partida",
                            y=[col_pp, col_pc],
                        ...
                        # Ajusta os nomes explicativos nas legendas laterais
                        ...
                        st.info("Nenhum dado válido sobrou após a limpeza de códigos ocultos.")
                else:
